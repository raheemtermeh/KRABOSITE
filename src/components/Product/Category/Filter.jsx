...
const Filter = ({
  ...
  ("Filter rendered at:", new Date().toLocaleTimeString());
  ("Product setting:", product?.data?.setting);
  ...
  ("Weight min/max from API:", MIN_WEIGHT, MAX_WEIGHT);
  ("Size min/max from API:", MIN_SIZE, MAX_SIZE);
  ...
  useEffect(() => {
    if (product?.data?.setting?.price && !isReady) {
      ...
      ("Price range:", [gtePrice, ltePrice]);
      ("Weight range:", [gteWeight, lteWeight]);
      ("Size range:", [gteSize, lteSize]);
